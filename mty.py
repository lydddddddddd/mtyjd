...
conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='Ini2Eash', db='mysql', charset='utf8')
cur = conn.cursor()
cur.execute('use mty')

url='http://www.tripadvisor.cn/Attractions-g294212-Activities-Beijing.html'
urls=['http://www.tripadvisor.cn/Attractions-g294212-Activities-oa{}-Beijing.html#ATTRACTION_LIST'.format(str(i)) for i in range(30,1020,30)]
headers={
...
def get_attractions(url,data=None):
    ...
    rates = soup.select('.ui_bubble_rating')
    for title, img, rate in zip(titles, imgs, rates):
        img_url = img.get('src')
...
def store(data):
    print(data['title'])
    cur.execute('select * from jingdian where title = %s', data['title'])
    row = cur.fetchone()
    if row == None:
        cur.execute('insert into jingdian (title, img, rate) values (%s, %s, %s)', (data['title'], pymysql.Binary(data['img']), data['rate']))
        cur.connection.commit()
    else:
        print('Item already exists: %s', data['title'])
